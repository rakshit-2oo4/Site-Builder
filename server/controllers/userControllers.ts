...
// Contrller Funcation to create new project
...
export const createUserProject = async (req: Request, res: Response) => {
    ...
    The HTML should be complete and ready to render as-is with Tailwind CSS.`,
        ...
        await prisma.conversation.create({
            data: {
                role: 'assistant',
                content: "I've generated your website! You con preview it and request any changes",
                projectId: project.id
            }
        });
        ...
        console.log("OpenRouter Error Details:", error.response?.data?.error || error.message);
        await prisma.user.update({
            where: { id: userId },
            data: { credits: { increment: 5 } }
        });
...
// Conteroller Function to get all user projects
...
//Controller Funcation to Toggle project Publish
...
// Controller Funcation to Purchase Credits
export const purchaseCredits = async (req: Request, res: Response) => {
    try {
        ...
        const session = await stripe.checkout.sessions.create({
            ...
            expires_at: Math.floor(Date.now() / 1000) + 30 * 60, // Expaire in 30 minutes
        });
