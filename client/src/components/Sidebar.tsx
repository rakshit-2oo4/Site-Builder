            ...
            <div className="flex flex-col h-full">
                ...
                <div className="flex-1 overflow-y-auto no-scrollbar px-3 flex flex-col gap-4">
                    ...
                    .sort((a,b)=> new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime()).map((message)=>{
                        ...
                        if(isMessage){
                            ...
                            <div className={`max-w-[80%] p-2 px-4 rounded-2xl shadow-sm text-sm mt-5 leading-relaxed ${isUser? "bg-linear-to-r from-indigo-500 to-indigo-600 text-white rounded-tr-none": "rounded-tr-none bg-gray-800 text-gray-100"}`}>
                                {msg.content}
                            </div>
                            {isUser && (
                                    ...
                                    <div className="flex items-center justify-between">
                                        {project.current_version_index === ver.id ? (
                                            ...
                                            <button onClick={() => handleRollback(ver.id)}
                                             className="px-3 py-1 rounded-md text-xs bg-indigo-500 hover:bg-indigo-600 text-white">Roll back to this version</button>
                                        )}
                ...
                <form onSubmit={handleRevision} className="m-3 relative">
                    <div className="flex items-center gap-2">
                        <textarea rows={4} placeholder="Describe your website o request changes..." disabled={isGenerating} onChange={(e)=> setInput(e.target.value)} value={input}
